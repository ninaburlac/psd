...
int main( int argc, char* argv[]){
  ...
  TH1D *hene = new TH1D("hene","; energy [keV]; Counts", 2000, 1001, 3000);
  
  TCanvas *c = new TCanvas("c","energy");
  gStyle->SetPalette(1);
  gStyle->SetOptStat(0); 
  gPad->SetLogy();
  tier->Draw(Form("energy[%d]>>hene",chn));
  c->Update();
  c->Print(Form("chn%d_spectrum.pdf",chn));
  
  //----- fit of the energy peaks with a double-gaussian --------
  
  TF1 *f_2gaus = new TF1("f_2gaus","gaus(0)+gaus(3)+pol0(6)",1570,1640);
  ...
  TCut cut_e = Form("energy[%d] > %g && energy[%d] < %g", chn, min_dep, chn, max_dep);
  TCut cut_e1 = Form("energy[%d] > %g && energy[%d] < %g", chn, min_dep2, chn, min_dep);
  TCut cut_e2 = Form("energy[%d] > %g && energy[%d] < %g", chn, max_dep, chn, max_dep2);
  
  //--------- find the value of the A/E ----------
  
  TH1D *AoE = new TH1D("AoE","; A/E; Counts",1000,0,0.1);
  tier->Draw(Form("AoE[%d]>>AoE",chn),cut_e);
  c1->Update();
  ...
  tier->Draw(Form("AoE[%d]/%g >>h_AoE", chn, mean_aoe),cut_e);
